...
const Influencers = ({influencer, loadInfluencers, auth, clearFilters}) => {
        ...
        <div>
            ...
            <Grid container spacing={4}>
                ...
                <Grid item xs={9}>
                    ...
                    <Box style={{color: 'white',  marginTop: '25px'}}>{ filtered !== null && filtered.length} Influencers Available</Box>
                    ...
                    <Grid container spacing={4}  direction="row" justify="flex-start" alignItems="flex-start">
                        { filtered !== null && filtered.map(i => (
                            <Grid key={i._id} item xs={9} lg={4} xl={4}>
                                <InfluencerCard displayName={i.user_info[0].display_name} level={i.level} verified={i.verified} description={i.card_description} rating={i.rating} price={i.starting_payment} socials={i.social_media} avatar={'/api/users/avatar/' + i.user} banner={'/api/influencers/banner/' + i.user}/>
                            </Grid>
                        ))}
...
export default connect(mapStateToProps, {loadInfluencers, setSearchTextBox, clearFilters}) (Influencers)
